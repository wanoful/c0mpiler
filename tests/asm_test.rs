...
#[test]
fn my_ir() {
    run_rv32_mir("testcases/IR", &[]);
}
#[test]
fn ir_1_asm() {
    run_rv32_mir("RCompiler-Testcases/IR-1", &[]);
}
#[test]
fn opti_local_asm() {
    run_rv32_mir("testcases/opti-local", &[]);
}

fn run_rv32_mir(case_path: &str, escape_list: &[&str]) {
    let reimu_path = std::env::var("REIMU_PATH").unwrap_or("reimu".to_string());
    let infos = load_test_infos(case_path);
    let temp = format!(
        "target/tmp/{}/asm",
        PathBuf::from(case_path).file_name().unwrap().display()
    );
    ...
    let out = Command::new("clang")
        ...
        .output()
        .expect("Failed to compile builtin.c");
    assert!(
        out.status.success(),
        "prelude failed:\n{}",
    ...
    for x in &infos {
        ...
        let asm = match with_frontend(&src, x.compileexitcode == 0, |analyzer, krate| {
            ...
            Ok(Err(e)) | Err(e) => {
                fault!(
                    true,
                    "{name} unexpected result (expect_pass={}): {e}",
                    x.compileexitcode == 0
                );
            }
            _ => unreachable!(),
        ...
        let in_arg = if PathBuf::from(&in_path).exists() {
            format!("-i={in_path}")
        } else {
            String::new()
        };
        let out_arg = if PathBuf::from(&out_path).exists() {
            format!("-a={out_path}")
        } else {
            String::new()
        };
        let mut args = vec![
            format!("-f={prelude_asm},{temp}/{name}.s"),
            "--oj-mode".into(),
            "-s=1M".into(),
        ];
        if !in_arg.is_empty() {
        ...
        match Command::new(&reimu_path).args(&args).output() {
            ...
            Err(e) => {
                fault!(true, "{name} reimu exec error: {e}");
            }
        }
...
#[test]
fn my_rv64_ir() {
    run_rv64_qemu("testcases/IR", &[]);
}
#[test]
fn ir_1_rv64_asm() {
    run_rv64_qemu("RCompiler-Testcases/IR-1", &[]);
}
#[test]
fn opti_local_rv64_asm() {
    run_rv64_qemu("testcases/opti-local", &[]);
}

fn run_rv64_qemu(case_path: &str, escape_list: &[&str]) {
    let qemu_path = std::env::var("QEMU_RV64_PATH").unwrap_or("qemu-riscv64".to_string());
    let infos = load_test_infos(case_path);
    let temp = format!(
        "target/tmp/{}/rv64_asm",
        PathBuf::from(case_path).file_name().unwrap().display()
    ...
    for x in &infos {
        ...
        let asm_out = Command::new("riscv64-linux-gnu-gcc")
            .args(["-c", &format!("{temp}/{name}.s"), "-o", &obj])
            .output();
        match asm_out {
            Ok(o) if o.status.success() => {}
            Ok(o) => {
        ...
        match ld_out {
            ...
            Err(e) => {
                fault!(true, "{name} linker error: {e}");
            }
        }

        let input_data = if PathBuf::from(&in_path).exists() {
        ...
        let mut child = match qemu {
            Ok(c) => c,
            Err(e) => {
                fault!(true, "{name} qemu exec error: {e}");
            }
        };
...
"#;
    ...
    let asm_out = Command::new("riscv64-linux-gnu-gcc")
        .args(["-c", &format!("{temp}/test.s"), "-o", &obj])
        .output()
        .unwrap();
    assert!(
