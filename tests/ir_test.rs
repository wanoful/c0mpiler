...
use c0mpiler::{
    ...
    irgen::IRGenerator,
    mir::lower::{LowerOptions, RV64Lowerer},
...
#[test]
fn my_ir() {
    if let Some(p) = option_env!("REIMU_PATH") {
        run_ir_reimu(p, "testcases/IR", &[]);
    } else {
        run_ir_native("testcases/IR", &[], true);
    }
}
#[test]
fn ir_1() {
    if let Some(p) = option_env!("REIMU_PATH") {
        run_ir_reimu(p, "RCompiler-Testcases/IR-1", &[]);
    } else {
        run_ir_native("RCompiler-Testcases/IR-1", &[], true);
    }
}
...
fn run_ir_native(case_path: &str, escape_list: &[&str], dry_run: bool) {
    ...
    for x in &infos {
        ...
        match out {
            ...
            Err(e) => {
                fault!(true, "{name} clang error: {e}");
            }
        }

        if !dry_run {
            let input = if PathBuf::from(&in_path).exists() {
